...
%%%%%%%%%%%%%%%%%%%%%
% total words: 1531 + 102 - 5 - 8 - 7 - 2 - 10 - 6 = 1595 %
%%%%%%%%%%%%%%%%%%%%%
...
\section*{Theoretical background}{
    ...
    \textit{Conditional relevance} \citep{Schegloff_1968} describes how one action makes certain next action(s) relevant (suitable, or even required for talk to proceed orderly), e.g. a question inviting an answer.
    ...
    Thus, long sequences consist of basic units -- \textit{adjacency pairs}; defined by \citet[p.~64]{Sidnell_2010} as pairs of elements:
    \begin{enumerate}
    	\item Adjacent
    ...
    \end{enumerate}
    The \textit{type} of a pair can be e.g. greeting-greeting or question-answer, an example pair of the latter type being:
    ...
    Importantly, an FPP can occasion many possible SPPs (including non-verbal ones, e.g. Betty showing John her wristwatch),
    ...
    Following his previous (preparatory) explanations of the pitiful situation, in \autoref{dispref-fpp} Jim provides a lengthy account before wording the actual request indirectly, using ``was wondering'' and ``could''.
